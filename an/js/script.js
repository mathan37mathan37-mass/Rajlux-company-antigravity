...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const revealObserver = new IntersectionObserver((entries) => {
    ...
    threshold: 0.01,
    rootMargin: '0px 0px 80px 0px' // triggers slightly before scrolling into view so text is ready immediately
  });
  ...
  function updateInquiryTypeUI(type) {
    ...
    if (projectOptionsRow) {
      projectOptionsRow.style.display = isProject ? '' : 'none';
    }
  ...
  if (form) {
    form.addEventListener('submit', async (e) => {
      ...
      if (isProjectInquiry) {
        ...
        // Clear form fields and reset state reliably
        setTimeout(() => {
          submitBtn.querySelector('span').textContent = originalText;
          submitBtn.disabled = false;
          ...
          if (document.getElementById('fservice')) document.getElementById('fservice').value = '';
          if (document.getElementById('fmessage')) document.getElementById('fmessage').value = '';
          clearAllErrors();
          updateInquiryTypeUI('project');
        }, 500);
      }
